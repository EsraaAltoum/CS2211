...
static Data *bst_search(BStree bst, int index, Key key) {
	if (index == 0 ) {
		...
		int comp = key_comp( bst->tree_nodes[i].key, key );
		if ( comp < 0 ) {
			return bst_search(bst, bst->tree_nodes[i].right, key);
		}
		else if ( comp > 0 ) {
			return bst_search(bst, bst->tree_nodes[i].left, key);
		}
		else
			return &bst->tree_nodes[i].data;
	}

}

// Input: 'bst' a binary search tree 
// Effect: returns the address of the key's associated data//

Data *bstree_search(BStree bst, Key key) {
	return bst_search(bst, bst->root, key);
}

// helper function for bstree_data_search//
static Key *bst_data_search(BStree bst, int index, Data data) {
	if (index == 0 ) {
		...
		Key* d;
		d = bst_data_search(bst, bst->tree_nodes[index].left, data);
		if(d != NULL) return d;

		if(data_comp(bst->tree_nodes[index].data, data) >= 0) return &(bst->tree_nodes[index].key);
					
		d = bst_data_search(bst, bst->tree_nodes[index].right, data);
		if(d != NULL) return d;
	}
	return NULL;
	}
...
//Input: 'bst' a binary search tree //
//Effect: returns the address of the key of the first node, in order of keys in BST//
